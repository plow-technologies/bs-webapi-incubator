...
let canvasEl = Document.createElement(document, "canvas");
...
let _ =
  List.map(
    createPattern(ctx, Document.createElement(document, "img")),
...
createImageDataFromImage(ctx, imageData)->ignore;
Image.width(imageData)->ignore;
Image.height(imageData)->ignore;

getImageData(ctx, ~sx=0.0, ~sy=0.0, ~sw=0.0, ~sh=0.0)->ignore;
...
ctx->fillText(~x=1., ~y=0., "hi")->ignore;
ctx->strokeText(~x=1., ~y=0., "hi")->ignore;

ctx->fillRect(~x=1., ~y=0., ~w=10., ~h=10.)->ignore;
ctx->strokeRect(~x=1., ~y=0., ~w=10., ~h=10.)->ignore;
ctx->clearRect(~x=1., ~y=0., ~w=10., ~h=10.)->ignore;
